    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional FTHT outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes FTHT and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::FTHT_TO_FTHT) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a FTHT-to-FTHT transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::FTHT_ONLY;
        ...
        // If FTHT-to-FTHT fails, create a peg-out transaction
