...
- [Development guidelines](#development-guidelines)
    - [General Fuckthemheggiescoin Core](#general-fuckthemheggiescoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Fuckthemheggiescoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Fuckthemheggiescoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/fuckthemheggiescoin-project/fuckthemheggiescoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_fuckthemheggiescoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_fuckthemheggiescoin --log_level=test_suite
$ valgrind -v --leak-check=full src/fuckthemheggiescoind -printtoconsole
...
# A coverage report will now be accessible at `./test_fuckthemheggiescoin.coverage/index.html`.
```
...
To profile a running fuckthemheggiescoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep fuckthemheggiescoind` -- sleep 60
```
...
Fuckthemheggiescoin Core can be compiled with various "sanitizers" enabled, which add
...
file descriptors, it will cause problems with Fuckthemheggiescoin's `select()` loop, because
...
```bash
$ lsof -p $(pidof fuckthemheggiescoind) |\
...
compatibility between nodes. This happened in Fuckthemheggiescoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Fuckthemheggiescoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Fuckthemheggiescoin Core, the wallet was always
